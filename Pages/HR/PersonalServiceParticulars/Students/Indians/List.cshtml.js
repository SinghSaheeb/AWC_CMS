$(document).ready(function () {
    ...
    var table = $('#indianStudentsTable').DataTable({
        paging: true,
        searching: true,
        ordering: true
    });

    // Load indian students on page load
    loadIndianStudents();

    // Load indian students into the table
    function loadIndianStudents() {
        $.ajax({
            url: '/api/indianstudents',
            type: 'GET',
            headers: {
        ...
        if (confirm('Are you sure you want to delete this officer?')) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    removeRowFromDataTable('#indianStudentsTable', row);
